...
import { GET_METHOD, HEADERS, REST_API_FILTER_SEARCH, REST_API_JQL_SEARCH, REST_API_USER_SEARCH } from "../constants/api-constants";

const searchIssuesByJQL = async (projects, linkType, issueKey, dateRange, fixedVersions, 
                                  skip, take, sprints, sort, team, groupFetching, status) => {
  ...
  const fullQuery = `${REST_API_JQL_SEARCH}?jql= ${params}&maxResults=${take}&startAt=${skip}`;
...
export const findChildByJql = async (projects, linkType, issue) => {
  ...
  let url = `${REST_API_JQL_SEARCH}?jql=${jqlFindChildByID}`;
...
export const findFilter = async (filterName) => {
  const response = await requestJira(
    `${REST_API_FILTER_SEARCH}?filterName=${filterName}`,
    {
      method: GET_METHOD,
      headers: HEADERS,
    }
...
export const getMyFilter = async () => {
  const response = await requestJira(`${REST_API_FILTER_SEARCH}`, {
    method: GET_METHOD,
    headers: HEADERS,
  });
  let result = await response.json();
...
export const getListUser = async () => {
  const response = await requestJira(`${REST_API_USER_SEARCH}`, {
    method: GET_METHOD,
    headers: HEADERS,
  });
